...
//    {
//        $sql = "UPDATE `bills` SET `date`= :date,`status`=:status,`totalPrice`=:totalPrice,`customer_id`= :customer_id WHERE id = :id";
//        $stmt = $this->database->prepare($sql);
//        $stmt->bindParam(':id',$bill->getId());
//        $stmt->bindParam(':date', $bill->getDate());
//        $stmt->bindParam(':status', $bill->getStatus());
//        $stmt->bindParam(':totalPrice', $bill->getTotalPrice());
//        $stmt->bindParam(':customer_id', $bill->getCustomerId());
...
//    }
    ...
    {
        $sql ="SELECT categories.name,products.name,products.price,details.quantity,bills.totalPrice,bills.status,customers.name,customers.phone,customers.email,customers.address FROM categories INNER JOIN products on categories.id = products.category_id
